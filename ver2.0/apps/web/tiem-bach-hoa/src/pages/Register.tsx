...
export default function RegisterPage() {
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      showSuccess('Đăng ký thành công với Google!');
      setTimeout(() => navigate('/'), 500);
    } catch (error: any) {
      console.error('Google sign up error:', error);
      showError('Đăng ký Google thất bại: ' + (error.message || 'Lỗi không xác định'));
    }
  };
  ...
  const handleFacebookSignUp = async () => {
    try {
      ...
      showSuccess('Đăng ký thành công với Facebook!');
      setTimeout(() => navigate('/'), 500);
    } catch (error: any) {
      console.error('Facebook sign up error:', error);
      showError('Đăng ký Facebook thất bại: ' + (error.message || 'Lỗi không xác định'));
    }
  };
