    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="font-heading text-lg font-bold">Thông tin giao hàng</h3>
          {addresses.length > 0 && (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <FormField form={form} name="fullName" label="Họ và tên" placeholder="Nguyễn Văn A" />
          <FormField form={form} name="email" label="Email" placeholder="example@gmail.com" type="email" />
        </div>
        <FormField form={form} name="phone" label="Số điện thoại" placeholder="090..." />
        <FormField form={form} name="address" label="Địa chỉ giao hàng" placeholder="Số nhà, đường, phường/xã..." />
        <FormField form={form} name="note" label="Ghi chú (tùy chọn)" placeholder="Lời nhắn cho shipper..." />
      ...
      <Button type="submit" disabled={createAddressMutation.isPending} className="h-12 w-full rounded-full font-bold">
