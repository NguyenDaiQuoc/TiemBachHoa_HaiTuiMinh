...
if (!empty($search)) {
    switch ($search_field) {
        case 'magiohang':
            $where_clause .= " AND tbl_hoadon.magiohang LIKE '%$search%'";
            break;
        case 'tenkhahchhang':
            $where_clause .= " AND tbl_dangky.tenkhahchhang LIKE '%$search%'";
            break;
        case 'dienthoai':
            $where_clause .= " AND tbl_dangky.dienthoai LIKE '%$search%'";
            break;
        case 'diachi':
            $where_clause .= " AND tbl_dangky.diachi LIKE '%$search%'";
            break;
        case 'trangthai':
            $status = ($search == 'đã xử lý' || $search == '1') ? 1 : 0;
            $where_clause .= " AND tbl_hoadon.trangthai = $status";
            break;
        default:
            $where_clause .= " AND (tbl_hoadon.magiohang LIKE '%$search%' 
                            OR tbl_dangky.tenkhahchhang LIKE '%$search%' 
                            OR tbl_dangky.dienthoai LIKE '%$search%'
                            OR tbl_dangky.diachi LIKE '%$search%')";
    }
}

$sql_lietke = "SELECT tbl_hoadon.*, tbl_dangky.tenkhahchhang, tbl_dangky.dienthoai, tbl_dangky.diachi, tbl_dangky.email 
               FROM tbl_hoadon 
            ...
            <form class="row g-3" method="GET" action="index.php">
                ...
                <div class="col-md-4">
                    <select name="search_field" class="form-select">
                        ...
                        <option value="magiohang" <?php echo $search_field == 'magiohang' ? 'selected' : ''; ?>>Mã đơn hàng</option>
                        <option value="tenkhahchhang" <?php echo $search_field == 'tenkhahchhang' ? 'selected' : ''; ?>>Tên khách hàng</option>
                        <option value="dienthoai" <?php echo $search_field == 'dienthoai' ? 'selected' : ''; ?>>Số điện thoại</option>
                        <option value="diachi" <?php echo $search_field == 'diachi' ? 'selected' : ''; ?>>Địa chỉ</option>
                        <option value="trangthai" <?php echo $search_field == 'trangthai' ? 'selected' : ''; ?>>Trạng thái</option>
                    </select>
                </div>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo $row['tenkhahchhang'] ?></td>
